...
class MainWidget(Widget):
    ...
    H_NB_LINES = 8
    H_LINES_SPACING = .15  # pourcentage sur la largeur de l'écran
    horizontal_lines = []
    SPEED = 2
    current_offset_y = 0
    ...
    def generate_tiles_coordinate(self):
        ...
        for i in range(len(self.tiles_coordinate), self.NB_TILES):
            self.tiles_coordinate.append((0, last_y))
            last_y += 1
    ...
    def get_line_x_from_index(self, index):
        ...
        offset = index - 0.5
        line_x = central_line_x + offset * spacing + self.current_speed_x
        return line_x
    ...
    def update_tiles(self):
        for i in range(0, self.NB_TILES):
            tile = self.tiles[i]
            tiles_coordinates = self.tiles_coordinate[i]
            xmin, ymin = self.get_tile_coordinate(tiles_coordinates[0], tiles_coordinates[1])
            xmax, ymax = self.get_tile_coordinate(tiles_coordinates[0] + 1, tiles_coordinates[1] + 1)
